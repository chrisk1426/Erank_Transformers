#!/usr/bin/env python3
"""Layer 0 MLP eRank evidence for standard modular addition.

Grouped bar chart showing R_mid^(0), R_post^(0), and Δ_MLP^(0) per seed,
plus mean ± std overlay. Demonstrates that the layer-0 MLP consistently
compresses (Δ_MLP < 0) across all seeds.
"""
...
def load():
    ...
    for s in SEEDS:
        ...
        with open(p) as f:
            d = json.load(f)
        mid.append(d["erank"]["layer_0"]["mid"])
        post.append(d["erank"]["layer_0"]["post"])
        delta.append(d["delta_erank_mlp"]["layer_0"])
...
def main():
    ...
    # ── Figure: grouped bars per seed + mean summary ──
    metrics = {
        "$R_{\\mathrm{mid}}^{(0)}$":  (mid,   COLOR_MID),
        "$R_{\\mathrm{post}}^{(0)}$": (post,  COLOR_POST),
        "$\\Delta_{\\mathrm{MLP}}^{(0)}$": (delta, COLOR_DELTA),
    }
    ...
    ax1.set_title("Layer 0 MLP eRank — Per Seed", fontsize=12)
    ...
    fig.suptitle("Standard Modular Addition — Layer 0 MLP Evidence",
                 fontsize=13, y=1.02)
    fig.tight_layout()
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    path = OUT_DIR / "layer0_mlp_erank_evidence.png"
